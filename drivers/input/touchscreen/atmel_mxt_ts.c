...
static int mxt_get_object_table(struct mxt_data *data)
{
	...
	dev_dbg(dev, "Information Block Checksum = %06x\n", data->info_csum);
		...
		const struct i2c_device_id *id)
{
	...
	ret = gpio_request(client->irq, "mxt224_irq");
	if (ret < 0) {
			     ...
			     unsigned int type, unsigned int code, int value)
{
	...
	if (type == EV_SW && code == SW_LID) {
		...
		if (value == 0) {
			...
			if (data->has_T9) {
				ret = mxt_save_regs(data, MXT_TOUCH_MULTI_T9,
					0, 0, &data->T9_ctrl, 1);
				if (ret)
					dev_err(dev, "Save T9 ctrl config failed, %d\n",
					ret);
			...
			if (data->has_T100) {
				ret = mxt_save_regs(data, MXT_TOUCH_MULTI_T100,
					0, 0, &data->T100_ctrl, 1);
				if (ret)
					dev_err(dev, "Save T100 ctrl config failed, %d\n",
					ret);
...
static struct of_device_id mxt_ts_dt_ids[] = {
...
static struct i2c_driver mxt_driver = {
	.driver = {
		...
		.of_match_table = of_match_ptr(mxt_ts_dt_ids),
	},
	.probe		= mxt_probe,
	.remove		= mxt_remove,
	.id_table	= mxt_id,
