...
struct rk32_hdmi {
	struct device *dev;
	struct rockchip_hdmi_driver drv;
	int irq;
...
static inline u32 hdmi_readl(struct rk32_hdmi *hdmi, u16 offset)
{
	return readl_relaxed(hdmi->regbase + (offset) * 0x04);
}

static inline int hdmi_writel(struct rk32_hdmi *hdmi, u16 offset, u32 val)
{
	int ret = 0;
...
static inline int hdmi_msk_reg(struct rk32_hdmi *hdmi, u16 offset,
			       u32 msk, u32 val)
{
	int ret = 0;
	u32 temp;
...
int rk3288_hdmi_initial(struct rk32_hdmi *hdmi);
void rk3288_hdmi_control_output(struct rk32_hdmi *hdmi, int enable);
int rk3288_hdmi_config_phy(struct rk32_hdmi *hdmiv, unsigned char pixel_repet,
