...
static struct regulator *midgard_regulator = NULL;
static int midgard_gpu_vol = 1250000;	/* 1.25V @ 480 MHz */
#endif
...
static mali_dvfs_info mali_dvfs_infotbl[] = {
      ...
      {900000,  300000, 60, 78, 0},
      {1000000, 420000, 65, 75, 0},
      {1100000, 500000, 70, 75, 0},
      {1250000, 700000, 90, 100, 0},
};
...
{
#ifdef CONFIG_REGULATOR
	if (!midgard_regulator) {
		printk(KERN_DEBUG "[kbase_platform_regulator_disable] midgard_regulator is not initialized\n");
		return -1;
	}

	if (regulator_disable(midgard_regulator) != 0) {
		printk(KERN_DEBUG "[kbase_platform_regulator_disable] failed to disable midgard_regulator\n");
		return -1;
	}
#endif
	return 0;
...
#ifdef CONFIG_REGULATOR
	if (!midgard_regulator) {
		printk(KERN_DEBUG "[kbase_platform_regulator_enable] midgard_regulator is not initialized\n");
		return -1;
	}

	if (regulator_enable(midgard_regulator) != 0) {
		printk(KERN_DEBUG "[kbase_platform_regulator_enable] failed to enable  midgard_regulator\n");
		return -1;
	}
#endif
	return 0;
...
int kbase_platform_regulator_init(void)
{
	...
	midgard_regulator = regulator_get(NULL, "vdd_gpu");
	if (IS_ERR(midgard_regulator)) {
		printk(KERN_DEBUG "[kbase_platform_regulator_init] failed to get mali t76x regulator\n");
		return -1;
	}
	...
	if (regulator_set_voltage(midgard_regulator, midgard_gpu_vol, midgard_gpu_vol) != 0) {
		printk(KERN_DEBUG "[kbase_platform_regulator_init] failed to set mali t76x operating voltage [%d]\n", mali_gpu_vol);
		return -1;
	}
#endif
	return 0;
...
int kbase_platform_dvfs_init(struct kbase_device *kbdev)
{
	...
	ret = dev_pm_opp_init_cpufreq_table(kbdev->dev, &mali_freq_table);
	if (ret) {
		dev_err(kbdev->dev,
		...
		for (i = 0; mali_freq_table[i].frequency != CPUFREQ_TABLE_END; i++) {
			...
			opp = dev_pm_opp_find_freq_exact(kbdev->dev,
						mali_freq_table[i].frequency * 1000, true);
			
			if (IS_ERR(opp)) {
				dev_err(kbdev->dev,
					"failed to find valid OPP for %u KHZ\n",
					mali_freq_table[i].frequency);
				return PTR_ERR(opp);
			}
...
#ifdef CONFIG_REGULATOR
	if (!midgard_regulator) {
		printk(KERN_DEBUG "[kbase_platform_get_voltage] midgard_regulator is not initialized\n");
		return -1;
	}

	*vol = regulator_get_voltage(midgard_regulator);
#else
	*vol = 0;
#endif
	return 0;
...
#ifdef CONFIG_REGULATOR
	if (!midgard_regulator) {
		printk(KERN_DEBUG "[kbase_platform_set_voltage] midgard_regulator is not initialized\n");
		return -1;
	}

	if (regulator_set_voltage(midgard_regulator, vol, vol) != 0) {
...
void kbase_platform_dvfs_set_clock(kbase_device *kbdev, int freq)
{
	...
	mali_dvfs_clk_set(platform->mali_sclk,freq);
