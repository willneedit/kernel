...
/*	printk("%s,line=%d\n", __func__,__LINE__);	*/
	...
	/*FIXME: maybe add pm_stay_awake*/
	rk808_i2c_read(rk808->i2c, RK808_INT_STS_REG1,  &reg);
	irq_sts = reg;
	rk808_i2c_read(rk808->i2c, RK808_INT_STS_REG2,  &reg);
	irq_sts |= reg << 8;
	rk808_i2c_read(rk808->i2c, RK808_INT_STS_MSK_REG1, &reg);
	irq_mask = reg;
	rk808_i2c_read(rk808->i2c, RK808_INT_STS_MSK_REG2, &reg);
	irq_mask |= reg << 8;

	irq_sts &= ~irq_mask;

	if (!irq_sts) {
		/*FIXME maybe add pm_relax*/
		return IRQ_NONE;
	}
	...
	reg = irq_sts & 0xFF;
	irq_sts >>= 8;
	rk808_i2c_write(rk808->i2c, RK808_INT_STS_REG1, reg);
	reg = irq_sts & 0xFF;
	rk808_i2c_write(rk808->i2c, RK808_INT_STS_REG2, reg);
	/*FIXME: maybe add pm_relax*/
	return IRQ_HANDLED;
}
...
/*	dev_info(rk808->dev, "%s,line=%d\n", __func__,__LINE__);	*/
	...
	rk808_i2c_read(rk808->i2c, RK808_INT_STS_REG2, &reg);
	rk808_i2c_write(rk808->i2c, RK808_INT_STS_REG2, reg);
	rk808_i2c_read(rk808->i2c, RK808_RTC_STATUS_REG, &reg);
	rk808_i2c_write(rk808->i2c, RK808_RTC_STATUS_REG, reg);
	...
	if (rk808->irq_gpio && !rk808->chip_irq) {
		...
		if (rk808->irq_gpio) {
			ret = devm_gpio_request_one(rk808->dev, rk808->irq_gpio,
				GPIOF_OUT_INIT_HIGH, "rk808_pmic_irq");
			if (ret) {
				dev_err(rk808->dev, "request of gpio %d failed, %d\n",
						rk808->irq_gpio, ret);
				return ret;
				}
	...
	rk808->irq_domain = irq_domain_add_linear(NULL, RK808_NUM_IRQ,
					&rk808_irq_domain_ops, rk808);
	if (!rk808->irq_domain) {
		dev_err(rk808->dev, "could not create irq domain\n");
		return -ENODEV;
	}
	ret = request_threaded_irq(rk808->chip_irq, NULL, rk808_irq,
		IRQF_TRIGGER_FALLING | IRQF_ONESHOT, "rk808", rk808);
